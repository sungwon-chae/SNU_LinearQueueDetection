...
@torch.no_grad()
def run(weights=ROOT / 'yolov5l-xs-1.pt',  # model.pt path(s)
    ...
    default_reg_line = np.array([0.15, 0.45])          # 줄 기본값
    queue_classifier = QueueClassifier(default_reg_line, reg_threshold, dis_threshold, 3)
    frame_count = 0
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if cls == 0:
                            ...
                            #cv2.rectangle(im0, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), color=(0,0,0), thickness=1, lineType=cv2.LINE_AA)
                      
                    if (det[:, -1] == 0).sum() != 0:
                        xybox_list = np.array(xybox_list)
                        
                        classified_xybox = queue_classifier.classify_pedestrians(xybox_list, im0)
                        for j in range(len(classified_xybox)):
                            label = f'in_queue{j}'
                            #annotator.box_label(xybox_list[j, 2:], label, color=(255, 255, 0))
                        
                        queue_line = queue_classifier.queue_line()
                        annotator.line(queue_line)
                        s += f'{sum(queue_classifier.is_in_line)} people in queue'  
                        print(queue_classifier.reg_line)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5l-xs-1.pt', help='model path(s)')
